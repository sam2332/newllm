...
from agent.turn_stream import TurnTracker
...
def make_grammar(tokenizer, allowed_names, constrained: bool):
    if not constrained:
        return None
...
def parse_json_turn(raw_model_text: str, allowed_names=None) -> TurnResult:
    """Interpret the model's output under the JSON protocol.
...
def _finish(result: TurnResult, ids: list, max_new: int, stats: dict):
    ...
    if result.fallback == "length" or (
            len(ids) >= max_new and ASSISTANT_CLOSE not in result.raw
            and EOT not in result.raw):
...
def generate_turn(model, context_tokens: list, *, sampler: Sampler,
    ...
    if protocol != "json":
        raise NotImplementedError(f"protocol {protocol!r} not implemented yet")
    _seed(seed)
    stats = {}
    ids = list(generate_tokens(model, context_tokens, sampler, max_new=max_new,
                               device=device, tokenizer=tokenizer,
                               stop_texts=stop_texts, grammar=grammar,
                               stats=stats))
    result = parse_json_turn(tokenizer.decode(ids), allowed_names)
    return _finish(result, ids, max_new, stats)
...
def stream_turn(model, context_tokens: list, *, sampler: Sampler,
    ...
    the grammar; a tool call is emitted whole; the final ``done`` event
    carries the complete ``TurnResult`` plus any text the deltas missed.
    """
    if protocol != "json":
        raise NotImplementedError(f"protocol {protocol!r} not implemented yet")
    _seed(seed)
    stats = {}
    tracker = TurnTracker(tokenizer, allowed_names)
    ids = []
    for token_id in generate_tokens(model, context_tokens, sampler,
                                    max_new=max_new, device=device,
                                    tokenizer=tokenizer, stop_texts=stop_texts,
                                    grammar=grammar, stats=stats):
        ids.append(token_id)
        for stream, text in tracker.feed(token_id):
            yield TurnEvent(type=stream, text=text)
    result = _finish(parse_json_turn(tokenizer.decode(ids), allowed_names),
                     ids, max_new, stats)
